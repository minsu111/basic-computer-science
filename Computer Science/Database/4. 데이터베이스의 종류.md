...
- [NoSQL 데이터베이스](#nosql-데이터베이스)
  - [MongoDB](#mongodb)
  - [Redis](#redis)
- [MySQL과 MongoDB의 비교](#mysql과-mongodb의-비교)
  - [커뮤니티 & 사용 현황](#커뮤니티--사용-현황)
  - [쿼리 & 성능](#쿼리--성능)
- [참고 자료](#참고-자료)
...
데이터베이스는 사용자와 데이터 사이에서 사용자의 요청에 따라 데이터의 생성, 조회 등 데이터베이스를 관리해주는 역할을 하는 시스템이다. 데이터베이스는 크게 관계형 데이터베이스(RDBMS)와 NoSQL 두 가지 주요 유형으로 분류된다.
...
- 의미: 데이터를 행과 열을 가지는 테이블 형태로 구조화하여 관리하는 데이터베이스, SQL이라는 언어를 써서 조작한다.
...
- **정규화를 진행한다**
  - 데이터 중복을 줄이는 등 이점이 있지만, 테이블이 여러 개로 나뉠수록 원래의 전체 데이터를 읽어오고자 할 때 JOIN이 많이 발생한다.
    - 여러 테이블 조인하려면 CPU 많이 써야하며 응답시간이 늘어난다.
    - 즉, 복잡한 join은 read 성능의 하락을 야기한다.
